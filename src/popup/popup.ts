...
function renderDevice(state: DeviceState): void {
  ...
  if (state.blocked) deviceEl.append(chip(t('chipBlocked'), 'danger'));
  else if (!state.known) deviceEl.append(chip(t('chipOffline'), 'warn'));
  else if (state.detectedBy === 'manual') deviceEl.append(chip(t('chipManual')));
...
function renderMenu(): void {
  ...
  addGroup(t('deviceGroupFavorites'), groups.favorites);
  addGroup(t(groups.favorites.length > 0 ? 'deviceGroupOthers' : 'deviceGroupAll'), groups.others);
  ...
  if (list.childElementCount === 0) {
    ...
    const text = document.createElement('span');
    text.textContent = t('noActiveDevices');
    empty.append(text);
    menuEl.append(empty);
    return;
  }
...
function renderOption(option: DeviceOption): HTMLLIElement {
  ...
  if (option.self) select.append(chip(t('markThisDevice')));
  else if (!option.active) select.append(chip(t('markOffline'), 'warn'));
...
function renderStar(option: DeviceOption): HTMLButtonElement {
  ...
  const label = option.favorite
    ? t('favoriteRemove', option.label)
    : t('favoriteAdd', option.label);
  ...
  const options: { id: string | null; label: string; sub?: string }[] = [
    {
      ...
      sub: defaultPolicyLabel
        ? t('policyDefaultSub', defaultPolicyLabel)
        : t('policyNoneSub'),
    },
...
async function selectDevice(mac: string): Promise<void> {
  ...
  setStatus(t('loadingDevicePolicies'), 'muted');
  await loadState(mac);
}
